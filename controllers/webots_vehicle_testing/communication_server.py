...
class CommunicationServer(object):
    ...
    def receive_blocking(self, client_socket):
        """Receives message in blocking mode. First 4 bytes of the message have to be the message length as uint32.
        ...
        try:
            ...
            while bytes_recd < required_len:
                ...
                chunks.append(chunk)
                if required_len == 64000 and len(chunk) >= 4:
                    msg_len = struct.unpack('I', chunk[0:4])[0]
                    if self.is_debug_mode:
                        print('Communication Server: Received msg length: {}'.format(msg_len))
                    required_len = msg_len + 4
